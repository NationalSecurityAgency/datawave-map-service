    ...
    <q-item-section>
      <q-item-label lines="1">
        <span
          ...
          :style="[monospaceLabel ? { fontFamily: 'monospace' } : {}]"
        >
          <q-btn
            v-if="showLabelCopyButton"
            icon="content_copy"
            flat
            round
            size="8px"
...
if (rawTypeName.value == GEO_QUERY_FEATURES) {
  ...
  if (simpleFeature.properties != undefined) {
    if ('term' in simpleFeature.properties) {
      ...
      displayLabel.value =
        '[' +
        simpleFeature.properties.range[0] +
        ', ' +
        simpleFeature.properties.range[1] +
        ']';
...
function showDeleteButton(): boolean {
  return (
    rawTypeName.value == GEO_QUERY_FEATURES || rawTypeName.value == GEO_FEATURES
  );
}
